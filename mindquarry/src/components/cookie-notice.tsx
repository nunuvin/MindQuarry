...
export default function CookieNotice({ enabled, message }: { enabled: boolean; message: string }) {
    ...
    const fullMessage = `${message} By continuing to use MindQuarry, you agree to the terms.`;
        ...
        <div className="fixed bottom-4 left-4 right-4 z-50 mx-auto max-w-3xl">
            <div className="soft-panel flex items-start justify-between gap-4 p-4 sm:p-5">
                <div>
                    ...
                    <p className="mt-2 text-sm leading-7 text-muted-foreground">{fullMessage}</p>
                </div>
                <button
                    type="button"
                    onClick={dismissNotice}
                    className="inline-flex h-10 shrink-0 items-center justify-center rounded-full border border-border/70 bg-card px-4 text-sm font-semibold text-foreground transition hover:border-sky-400/70 hover:text-foreground"
                    aria-label="Dismiss cookie notice"
                >
                    <X className="mr-2 h-4 w-4" /> Continue
                </button>
            </div>
        </div>
    );
